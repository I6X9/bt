...
/*
 * //////////////////////////////////////////////////////////////////////////////
 * ä¼ è¾“å±‚ç±»åž‹ã€‚å½“å‰è¿˜ä¸æ”¯æŒiAPã€‚
 */
...
typedef enum _ErrorCode {
  ...
  ERROR_CODE__BUSY = 7,
  /*
   *Pairçš„é”™è¯¯è¿”å›žï¼šå·²ç»è¢«é…å¯¹äº†
   */
...
typedef enum _InitiatorType {
  /*
   *å¯é€‰çš„å€¼ï¼š"PRESS_AND_HOLD", "TAP", "PHONE_WAKEUP"
   */
 ...
 * //////////////////////////////////////////////////////////////////////////////
 * æŽ§åˆ¶æŒ‡ä»¤å°è£…
 * æ¯ä¸€ä¸ªå‘é€åˆ°æŽ§åˆ¶æµçš„ä¿¡æ¯éƒ½éœ€è¦è¢«åŒ…åœ¨ControlEnvelopeé‡Œé¢ã€‚è¯¦ç»†å®šä¹‰å¦‚ä¸‹ã€‚
 */
struct  _ControlEnvelope
{
  ...
  Command command;
  /*
   *ä¸»åŠ¨å‘èµ·çš„commandï¼Œå¸¦ä¸Šè¿™ä¸ªå‚æ•°ï¼ŒAckçš„commandä¼šç…§åŽŸæ ·å¸¦å›žæ¥ï¼Œå»ºè®®é•¿åº¦ä¸è¶…è¿‡6ä¸ªå­—èŠ‚
   */
  char *request_id;
  /*
   *sign2ä¸Žrand2çš†ä¸ºå¯é€‰å­—æ®µï¼Œç”¨äºŽæ‰‹æœºå‘èµ·çš„æŒ‡ä»¤çš„ç­¾åï¼Œåœ¨å¤–è®¾å¯¹æ­¤å­—æ®µåšç›¸åº”çš„é‰´æƒæ“ä½œï¼Œå…·ä½“æ¸…ç©ºREADME.mdä¸­å…³äºŽé‰´æƒçš„æè¿°
   *ç›®å‰ä»…æœ‰PROVIDE_SPEECHå’ŒGET_STATE(ENTER_OTA_MODE)éœ€è¦å¸¦sign2å­—æ®µ
   *sign2 = sha256(rand2 + rand + key + product\_id + serial\_number)
   *ç­¾åç®—æ³•éœ€è¦ä½¿ç”¨PairInformationä¸­çš„SignMethod
   *èµ·åsign2å’Œrand2æ˜¯ä¸ºäº†åŒºåˆ†PairInformationä¸­çš„signå’Œrand
   */
  char *sign2;
  /*
   *å¯é€‰
...
/*
 * æ¯ä¸€ä¸ªå‘é€åˆ°æŽ§åˆ¶æµçš„æŽ§åˆ¶æŒ‡ä»¤éƒ½éœ€è¦æœ‰ä¸€ä¸ªå›žå¤ï¼ˆACKï¼‰ã€‚
 */
struct  _Response
{
  ProtobufCMessage base;
...
struct  _DeviceInformation
{
  ...
  /*
   *è®¾å¤‡æ”¯æŒçš„è¯­éŸ³äº¤äº’æ–¹å¼
   */
  InitiatorType initiator_type;
  /*
   *äº§å“idéœ€è¦ä»Žåº¦ç§˜å¹³å°èŽ·å¾—, ä¸ºç©ºçš„æ—¶å€™ï¼Œåˆ¤å®šè®¾å¤‡ä¸ºUNKONWNæˆ–è€…OTHERã€‚ é•¿åº¦ä¸è¶…è¿‡32å­—èŠ‚
   */
  char *product_id;
  /*
   *ç±»ä¼¼06:36:3b:cd:c1:b0
   */
  char *classic_bluetooth_mac;

  /*
   *ÒÆ¶¯µ½ÁËPairInformation£¬ÎªÁË²»ÒýÆð¼æÈÝÎÊÌâ£¬ÒÔÏÂ13¡¢14²»ÒªÔÙÊ¹ÓÃÁË
   *string rand=13;//³¤¶È8Î»µÄËæ»ú×Ö·û´®¡£×¢Òâ£ºÐèÒªÃ¿´Î·µ»Ø¶¼²»Ò»Ñù¡£¾ßÌåÂß¼­Çë¿´README.mdÖÐµÄ¼øÈ¨²¿·Ö
   *string sign=14;//sign = sha256(rand + key + product_id + serial_number),  keyÐèÒª´Ó¶ÈÃØÆ½Ì¨»ñµÃ¡£¾ßÌåÂß¼­Çë¿´README.mdÖÐµÄ¼øÈ¨²¿·Ö
   */
  /*
   *Èç¹ûÕâ¸ö×Ö¶ÎÎªfalse£¬ÊÖ»ú¶Ë»áÃ¿1-2Ãë·¢Ò»¸öGET_STATEµÄÇëÇó£¬×÷ÎªÉè±¸µÄÐÄÌø°ü¡£ÉèÎªtrueµÄÊ±ºò£¬ÊÖ»ú²»·¢ËÍÐÄÌøÇëÇó
   */
  ...
  char *ota_version;
	...
	/* Èç¹ûÎªTURE£¬Ôò²»ÔÙÇ¿µ÷Éè±¸±ØÐëÁ¬A2DP */
  protobuf_c_boolean no_a2dp;

  /*
   *Èç¹ûÕâ¸ö×Ö¶ÎÎªtrue£¬Éè±¸²»Ö§³ÖDMAµÄForwardATCommandÖ¸Áî£¬ÕâÑùÐ¡¶ÈappÔÚ³¢ÊÔ°ïÓÃ»§²¦´òµç»°µÄÊ±ºò£¬²»»áÊ¹ÓÃForwardATCommand£¬¶øÊÇÊ¹ÓÃios»ò°²×¿ÏµÍ³µÄ²¦ºÅapi
   */
...
struct  _PairInformation
{
  ProtobufCMessage base;
  /*
   *é•¿åº¦8ä½çš„éšæœºå­—ç¬¦ä¸²ã€‚æ³¨æ„ï¼šéœ€è¦æ¯æ¬¡è¿”å›žéƒ½ä¸ä¸€æ ·ï¼Œä¸€æ—¦è¿”å›žåŽï¼Œéœ€è¦æŠŠè¿™ä¸ªå€¼ä¿å­˜ä¸‹æ¥ï¼Œä¾›ProvideSpeechçš„ç­¾åéªŒè¯ä½¿ç”¨ã€‚å…·ä½“é€»è¾‘è¯·çœ‹README.mdä¸­çš„é‰´æƒéƒ¨åˆ†
   */
  char *rand;
  /*
   *sign = sha256(rand + key + product_id + serial_number),  keyéœ€è¦ä»Žåº¦ç§˜å¹³å°èŽ·å¾—ã€‚å…·ä½“é€»è¾‘è¯·çœ‹README.mdä¸­çš„é‰´æƒéƒ¨åˆ†
   */
  char *sign;
  /*
   *ç­¾åç®—æ³•
