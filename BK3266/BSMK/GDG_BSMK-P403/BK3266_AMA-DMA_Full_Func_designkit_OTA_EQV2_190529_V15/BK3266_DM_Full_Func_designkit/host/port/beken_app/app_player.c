...
{	
    ...
    if(err == ERR_MP3_NONE )
    {
		...
		if (flag_L_is_LplusR)
			LeadelityMono(mp3_pcm_ptr_tmp, mp3_pcm_ptr_tmp, pcm_size/2, 44100, 0);
		else
			Leadelity(mp3_pcm_ptr_tmp, mp3_pcm_ptr_tmp, pcm_size/2, 44100, 0);
	#endif
		...
		int32 p;
		int16 *ptr;
		int32 *data_tmp = (int32 *)jmalloc(pcm_size*sizeof(uint32), 0);

		ptr = (int16 *)mp3_pcm_ptr_tmp;
		if((aud_pre_eqe.online_flag == 0x5a)||(aud_pre_eqe.online_flag == 1))
		{
			for(p=0;p<pcm_size;p++)
				data_tmp[p] = ptr[p]*aud_pre_eqe.globle_gain;
			func_sw_eq((int32 *)data_tmp,(int32 *)data_tmp, pcm_size/2, aud_pre_eqe.totle_EQ,0);
		...
		else if(app_drc.flag)
		{
			...
			if((app_drc.flag)&(1<<2))
				drc_apply(&app_drc, (int16_t*)mp3_pcm_ptr_tmp, (int16_t*)mp3_pcm_ptr_tmp, pcm_size/2);
...
static void mp3_mem_init(void)
{
    ...
    readBuf= (unsigned char*)jmalloc(READBUF_SIZE, M_ZERO);   //FOR READ BUF
