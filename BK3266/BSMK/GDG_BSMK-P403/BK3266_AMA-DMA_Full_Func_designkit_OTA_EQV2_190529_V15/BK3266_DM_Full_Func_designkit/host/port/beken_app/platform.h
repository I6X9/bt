...
#define DUEROS_WAIT(x)          //  os_delay_ms(x)//osSignalWait((1 << AF_FADE_OUT_SIGNAL_ID), osWaitForever)
#define DUEROS_WAKEUP(tid) //osSignalSet(tid, AF_FADE_OUT_SIGNAL_ID)   
...
#define DUEROS_StoreBE8(buff,num) ( ((buff)[0] = (uint8) ((num)) ) )

#define DUEROS_StoreBE16(buff,num) ( ((buff)[0] = (uint8) ((num)>>8)),    \
            ((buff)[1] = (uint8) (num)) )

#define DUEROS_StoreBE32(buff,num) ( ((buff)[0] = (uint8) ((num)>>24)),  \
                              ((buff)[1] = (uint8) ((num)>>16)),  \
                              ((buff)[2] = (uint8) ((num)>>8)),   \
                              ((buff)[3] = (uint8) (num)) )
...
#define DUEROS_MALLOC(size)        jmalloc(size,0)
#define DUEROS_FREE(ptr)              jfree(ptr)
