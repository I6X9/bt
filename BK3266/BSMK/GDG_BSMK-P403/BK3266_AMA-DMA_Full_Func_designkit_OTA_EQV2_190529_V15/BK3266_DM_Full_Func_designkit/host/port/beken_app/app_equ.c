...
void app_eq_get_on_line_para(int index,uint8 *ptr)
{
	int a0, a1, b0, b1, b2, b3;

	memcpy( (uint8 *)aud_equ[index].a, (uint8 *)ptr, 4*5);
	aud_equ[index].flag_enable = 1;
	equ_enable_count = aud_equ[0].flag_enable + aud_equ[1].flag_enable
		+ aud_equ[2].flag_enable + aud_equ[3].flag_enable + aud_equ[4].flag_enable;

	b0 = aud_equ[index].a[0];
	b1 = aud_equ[index].a[1];
	b2 = aud_equ[index].b[0];
	a0 = aud_equ[index].b[1];
	a1 = aud_equ[index].b[2];

	aud_equ[index].a[0] = a0>>7;
	aud_equ[index].a[1] = a1>>7;
	aud_equ[index].b[0] = b0>>7;
	aud_equ[index].b[1] = b1>>7;
	aud_equ[index].b[2] = b2>>7;

	aud_equ[index].a[0] = -aud_equ[index].a[0];
	aud_equ[index].a[1] = -aud_equ[index].a[1];
	//app_equalizer_a2dp_apply();
	
	os_printf("HW0=0x%x,0=0x%x,,1=0x%x,2=0x%x,3=0x%x,4=0x%x OK\n",index,aud_equ[index].a[0],aud_equ[index].a[1],aud_equ[index].b[0],aud_equ[index].b[1],aud_equ[index].b[2]);
}
