...
static void app_fm_info_save(uint8 write_now_Flag)
{
    ...
    if(write_now_Flag)
        ...
        jtask_schedule(app_h->app_save_env_task,3000,app_env_fm_wr_flash,NULL);
}
        ...
        else
        {
            ...
            switch(msg.id)
            {
                ...
                case MSG_INPUT_TIMEOUT:
                    /*由红外界面返回*/
                    if (input_number <= env_h->env_data.valid_channel_count)							//输入的是台号
                    {
                        if(env_h->env_data.freq_deviation_array[input_number] > MAX_FRE_DEVIATION)
                        {
                            sw_channel_index = 0;
                        }
                        else
                        {
                            env_h->env_data.valid_channel_idx = input_number;
                            sw_channel_index = env_h->env_data.freq_deviation_array[env_h->env_data.valid_channel_idx];
                        }

                        BK3252_FMSeek_Software(sw_channel_index + MIN_FRE);
                        aud_PAmute_oper(0);

                    }
                    else if ((input_number >= MIN_FRE) && (input_number <= MAX_FRE)) //输入的是频点
                    {
                    sw_channel_index = input_number - MIN_FRE;
                    BK3252_FMSeek_Software(input_number);
                    aud_PAmute_oper(0);
                    }
                    else
                    {
