...
extern void aud_mute_update_led( int16 samplel, int16 sampler );
    ...
    if(decode_frms > 0)
    {
       ...
       ************************************/
    		...
    		if(!bt_flag1_is_set(APP_FLAG_WAVE_PLAYING))
            {
                ...
                int a;
                for( a = 0; a < 256; a+= 16 )
                	aud_mute_update_led( sample[a], sample[a+1] );
				...
				if (flag_L_is_LplusR)
					LeadelityMono((short*)app_sbc.sbc_output,(short*)app_sbc.sbc_output, 512/4, 44100, 0);
				else
					Leadelity((short*)app_sbc.sbc_output,(short*)app_sbc.sbc_output, 512/4, 44100, 0);
				#endif
