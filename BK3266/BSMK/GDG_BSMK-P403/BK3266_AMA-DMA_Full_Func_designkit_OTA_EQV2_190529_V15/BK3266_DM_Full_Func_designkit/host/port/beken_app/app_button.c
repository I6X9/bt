...
static uint32 buttonplayEvent = MSG_PLAY_BUTTON_TONE;
...
uint8 button_play_flag = 0;
uint8 button_play_status = 0;
uint32 buttonDetectIO(void)
{
	...
	if(keyIOmap)
	{
		if(button_play_flag)
		{
			...
			{
				button_play_status = 0;
				msg_put(buttonplayEvent);
