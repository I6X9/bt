...
void app_env_cali_data_init(void)
{
	...
	{
		if(flash_get_16M_flag())
		{
			os_printf("Flash is 16M\r\n");
			env_cali_ptr = (uint8 *)FLASH_ENVDATA2_DEF_ADDR_16M;	
			flash_read_data(chip_magic, (uint32)env_cali_ptr, sizeof(env_chip_magic));
			if(os_memcmp(chip_magic,env_chip_magic,sizeof(env_chip_magic))) 
			{
				tlv_cali_falg = 0;
...
CONST uint8 addr_table_data[16] = {0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46};
extern uint8 LE_name_addr[4];
extern uint8 LEconfig_device_address[6];
...
void app_env_init( void )
{
	...
	addr_l = env_h->env_cfg.bt_para.device_addr.b[0];
	addr_h = env_h->env_cfg.bt_para.device_addr.b[1];

	LE_name_addr[0] = addr_table_data[(addr_l>>0) & 0x0f];
	LE_name_addr[1] = addr_table_data[(addr_l>>4) & 0x0f];
	LE_name_addr[2] = addr_table_data[(addr_h>>0) & 0x0f];
	LE_name_addr[3] = addr_table_data[(addr_h>>4) & 0x0f];
...
#ifndef TIANMAO_ENABLE
	j = string_size((uint8*)&env_h->env_cfg.bt_para.device_name[0]);	
	env_h->env_cfg.bt_para.device_name[j+0] = ' ';
	env_h->env_cfg.bt_para.device_name[j+1] = LE_name_addr[3];
	env_h->env_cfg.bt_para.device_name[j+2] = LE_name_addr[2];
	env_h->env_cfg.bt_para.device_name[j+3] = LE_name_addr[1];
	env_h->env_cfg.bt_para.device_name[j+4] = LE_name_addr[0];
	env_h->env_cfg.bt_para.device_name[j+5] = 0;
#endif
	...
	if (!(env_h->env_cfg.system_para.system_flag & APP_ENV_SYS_FLAG_UARTDBG))
        {
			#if (CONFIG_XIAONIU_DEVICE_TEST == 1)
			// XIAONIU test enable  and UART DUGBU disable set Tx is High impedance
		    gpio_config(0,5);
			#else
			// XIAONIU test disable and UART DUGBU disable set TX/RX is High impedance
			gpio_config(0,5);
			gpio_config(1,5);
	    	#endif
		}
...
//	app_env_dump();
	...
	uint16 pd_volt = env_h->env_cfg.system_para.lp_pd_threshold;
        ...
        #elif (POWERKEY_5S_PARING == 1)
            uint32 t0,t1;
            t0 = os_get_tick_counter();
            //bt_flag1_operate(APP_FLAG_MATCH_ENABLE,0);
            while(1)
            {
                t1 = os_get_tick_counter();
                if( (t1 - t0) <=  100)//1s开机
                {
                	if( gpio_input(wakup_pin) != 1 )
                	{
                	#if ((CONFIG_CHARGE_EN == 1))
                	    if (GPIO_CHARGER_FLAG)
                        {
