...
extern uint32 dac_check;

extern uint8 button_play_flag;
extern uint8 button_play_status;


uint8 le_connected_first = 0;

app_rgb_info_t rgb_info;


#ifdef BT_DUALMODE
extern u_int8 LEconnection_LE_Connected(void);
//extern u_int8 user_define_notify_handle;
extern void app_gatt_write_local_value(u_int16 char_val_handle,u_int8 length, u_int8* p_attrib_value);
#endif

extern volatile uint8 f_opus_encoded;
extern  void _Stack_Integrity_Check(void);

uint8 app_online_flag=0;
...
//static  int app_button_lang_change( void );
  ...
  int app_button_redial_last_number( void );
...
#endif
  int app_button_hf_transfer_toggle( void );
#endif
...
{// return 1:enable set_LE_Advertise           return 0:disable LE_Advertise
	...
	if (flag_ble_enable && flag_not_autoconn_ble_enable)
...
//    {
    ...
    if(hci_get_acl_link_count(app_h->unit))
    {
        bt_flag2_operate( APP_FLAG2_MATCH_BUTTON, 1);
   	  a2dp_cmd_disconnect();   //修改华为手机断开慢的问题
    	 os_delay_ms(200);
    #ifdef BT_ONE_TO_MULTIPLE
...
void power_rgb_init(void)
{
	gpio_config( GPIO12, 1 ); 	
	gpio_output(GPIO12, 1);
	rgb_info.enable = 0;
	rgb_info.mode = 0;
}

void set_rgb_color(uint8 color_r,uint8 color_g,uint8 color_b)
{
	if(color_r)
	{
		gpio_config(RGB_R_PIN, 1);
		gpio_output(RGB_R_PIN, RGB_R_ON);
	}
	else
	{
		gpio_config(RGB_R_PIN, 1);
		gpio_output(RGB_R_PIN, RGB_R_OFF);
	}

	if(color_g)
	{
		gpio_config(RGB_G_PIN, 1);
		gpio_output(RGB_G_PIN, RGB_G_ON);
	}
	else
	{
		gpio_config(RGB_G_PIN, 1);
		gpio_output(RGB_G_PIN, RGB_G_OFF);
	}

	if(color_b)
	{
		gpio_config(RGB_B_PIN, 1);
		gpio_output(RGB_B_PIN, RGB_B_ON);
	}
	else
	{
		gpio_config(RGB_B_PIN, 1);
		gpio_output(RGB_B_PIN, RGB_B_OFF);
	}
}


void app_rgb_mode_handle(void)
{
    app_handle_t app_h = app_get_sys_handler();
 	...
 	if (rgb_info.enable)
	{
		...
		{
			case RGB_RED:				
				set_rgb_color(RGB_R_ON,RGB_G_OFF,RGB_B_OFF);				
 				break;

			case RGB_GREEN:
				set_rgb_color(RGB_R_OFF,RGB_G_ON,RGB_B_OFF);
				break;

			case RGB_BLUE:
				set_rgb_color(RGB_R_OFF,RGB_G_OFF,RGB_B_ON);
				break;

			case RGB_YELLOW:
				set_rgb_color(RGB_R_ON,RGB_G_ON,RGB_B_OFF);
				break;

			case RGB_MAGENTA:
				set_rgb_color(RGB_R_ON,RGB_G_OFF,RGB_B_ON);
				break;

			case RGB_CYAN:
				set_rgb_color(RGB_R_OFF,RGB_G_ON,RGB_B_ON);
				break;

			case RGB_WHITE:
				set_rgb_color(RGB_R_ON,RGB_G_ON,RGB_B_ON);
				break;

			case RGB_NO:
				set_rgb_color(RGB_R_OFF,RGB_G_OFF,RGB_B_OFF);	
				break;
...
void app_button_rgb_mode(void)
{
	...
	if((rgb_info.mode == 8)||(rgb_info.mode == 0))
	{
		CLEAR_PWDOWN_TICK;
		//os_printf("app_button_rgb_mode:pwdown_tick = %d.\r\n",pwdown_tick);
	}
	...
	os_printf("%s:mode %d.\r\n", __func__,rgb_info.mode);
}

uint8 rgb_r,rgb_g,rgb_b;
uint8 dac_lev =0;

void app_breathing_light(void)
{
	 static uint16 Counter = 0,Compare=0,Direction=1,display_cnt = 0;
	 app_handle_t app_h = app_get_sys_handler();
	 static uint8 rgb_r_tmp,rgb_g_tmp,rgb_b_tmp;

	if(rgb_info.mode != 0)
		return;
	
	if((!( app_h->flag_sm1 & APP_BUTTON_FLAG_PLAY_PAUSE))||(dac_check <= 0))
	{
		set_rgb_color(rgb_r_tmp,rgb_g_tmp,rgb_b_tmp);
		return;
	}
	...
	if(Counter>Compare)
	{
		...
		rgb_g_tmp = rgb_g;
		rgb_b_tmp = rgb_b;
		set_rgb_color(rgb_r,rgb_g,rgb_b);		
	}           
	else
	{
		set_rgb_color(RGB_R_OFF,RGB_G_OFF,RGB_B_OFF);
	...
	{
		display_cnt = 0;
		//os_printf("%s:display_cnt = %d.rgb_r = %d.rgb_g = %d.rgb_b = %d.\r\n", __func__,display_cnt,rgb_r,rgb_g,rgb_b);
		get_random_color(dac_check,&rgb_r,&rgb_g,&rgb_b);
		dac_lev = get_dac_level(dac_check);
		//os_printf("%s:dac_check = %d.rgb_r = %d.rgb_g = %d.rgb_b = %d.dac_lev = %d.\r\n", __func__,dac_check,rgb_r,rgb_g,rgb_b,dac_lev);
	}
#endif   
}

uint8 get_dac_level(uint32 dac_val)
{
	uint8 i,rt_val;

	for(i = 0;i < 15;i++)
	{
		if((dac_val > 1600*i)&&(dac_val < (1600*(i+1))))
		{
			switch(i)
			{
				case 1:
...
void get_random_color(uint32 dac_val,uint8 *color_r,uint8 *color_g,uint8 *color_b)
{
	if(((dac_val%1000)/100)%2)
	{
		*color_r = 1;//R OPEN
	}
	else
...
//#endif
...
#if 1//def SUPPORT_BAIDU_OPUS
	if(voice_key_time_out>0)
	return -1;
	voice_key_time_out = 2;
    app_handle_t app_h = app_get_sys_handler();

    if (app_h->button_type != BUTTON_TYPE_NON_HFP)
    	return -1;
	le_connected_first = 0;
	...
	//if(LEconnection_LE_Connected())
	//	start_wave_and_action(APP_WAVE_FILE_ID_AILI_SCO_START, dueros_dma_start_speech);
	//else if (spp_is_connected())
	//	start_wave_and_action(APP_WAVE_FILE_ID_AILI_SCO_START, dueros_dma_start_speech);
...
/*static*/ int app_button_powerdown(void)
{
    ...
    app_env_set_sw_power_down_flag(0x01);
#endif
...
static int app_powerdown_condition_addtick( void )
{
    ...
    if (SYS_WM_BT_MODE == app_h->sys_work_mode)
    {
        if(!(bt_flag1_is_set(APP_FLAG_DUT_MODE_ENABLE | APP_FLAG_FCC_MODE_ENABLE)|| get_Charge_state()))
        {	
		#ifdef BEKEN_OTA
		extern uint8 app_ota_is_updating(void);// OTA正在升级
...
static int app_powerdown_condition_detect(void)
{
    ...
    if (((app_h->powerdown_count>0)
			...
			&&((rgb_info.mode == 0)||(rgb_info.mode == 8))
            )
...
u_int8  ota_update_flag;
volatile uint8 flag_ota_beginning;
...
#endif
uint8 button_play_cnt = 0;
u8 led_voice_flag;
uint16 voice_no_wakeup = 0;
void app_button_led_action( uint32 step )
{
	//app_handle_t app_h = app_get_sys_handler();
    app_env_handle_t env_h = app_env_get_handle();
	static uint8 led_flash_cnt=0;
#ifdef BEKEN_OTA
	//app_ota_time_out_action();// OTA升级超时复位
#endif
    if(le_connected_first > 0)
    	le_connected_first --;
    app_led_action(step);
	...
	if(led_flash_cnt == 200)
	{
	 	led_flash_cnt = 0;
		dac_lev = get_dac_level(dac_check);
		//os_printf("dac_check = %d.\r\n",dac_check);
	  	//os_printf("rgb_r = %d.rgb_g = %d.rgb_b = %d.\r\n",rgb_r,rgb_g,rgb_b);
	}
	if(button_play_status == 0)
	{
		button_play_cnt++;
		if(button_play_cnt == 15)
		{
			button_play_cnt = 0;
			button_play_status = 1;
		}
	}
...
extern app_wave_handle_t app_get_wave_handle( void );
extern uint8 voice_start_flag;
void app_audio_restore(void)
{
	...
	{
		//ALI提示音结束后打开MIC
		if (wave_h->file_id == APP_WAVE_FILE_ID_AILI_SCO_START)
		{
...
#ifdef SUPPORT_BAIDU_OPUS	
uint8 send_dma_ver=0;
uint8  SAMPLE_ALIGN dma_rev_buf[160];
#endif
...
void bt_mode_msg_handler(void)
{
    ...
    while(!exit_flag)
    {
    ...
    #endif
        {
	   		...
	   		/* 在回连时不开BLE防止有些安卓手机回连不成功 */
			if(bt_flag1_is_set(APP_FLAG_AUTO_CONNECTION|APP_FLAG_RECONNCTION))
				flag_not_autoconn_ble_enable = 0;	
			else
				flag_not_autoconn_ble_enable = 1;	
			...
			rev_len=get_que(dma_rev_buf);
			if(rev_len)
			{
				//os_printf("ble_revece_leng=%d\r\n",ble_revece_leng);			
				dueros_process_cmd_from_phone((uint8*)dma_rev_buf,rev_len);	
			}
        ...
        else
        {
			...
			case MSG_START_OPUS_ENCODE:
				if (! spp_is_connected())
			 		en_opus();
			  break;

	          case MSG_KEY_MUTE:
	          case MSG_KEY_VOICE_START:
		      ...
		      break;
                ...
                case MSG_HALF_SECOND:
                    break;
				case MSG_KEY_RESERVED1:
 					app_button_rgb_mode();
					break;	
				case MSG_PLAY_BUTTON_TONE:
					start_wave_and_action(APP_WAVE_FILE_ID_VOICE_NUM_0, NULL);	
					break;
                default:
...
//wait WAV and BT disconnect end	
    ...
    while(1)
    {
        BT_msg_handler();
        app_wave_file_play();   
        if(app_wave_playing()||bt_flag1_is_set(APP_AUDIO_FLAG_SET))
        {
...
void api_loop()
{
	static uint8  f_auto_connt=0;
	static uint8  dam_time_out=0;
	...
	if(( bt_flag1_is_set(APP_FLAG_HFP_CALLSETUP)||(bt_flag1_is_set(APP_FLAG_SCO_CONNECTION)))&&(otaupflag))
	{
		hf_sco_handle_process(1);
	}
...
{
//os_printf("app_get_flow_ctrl_en\r\n");
 return  sbc_disable;
...
volatile uint8 flag_os_printf_disable = 0;
volatile uint8 user_batt_charger_vlcf = 0xff;
volatile uint16 eeprom_block_num;


#define EEPROM_BLOCK_ID1      	0x55
#define EEPROM_BLOCK_ID2      	0xaa
#define EEPROM_BLOCK_SIZE      	4
#define EEPROM_WHOLE_SIZE      	1024
#define EEPROM_BLOCK_NUMBER     (EEPROM_WHOLE_SIZE/EEPROM_BLOCK_SIZE)


void rf_test_user_set_reg(void)
{
	BK3000_AE_CONFIG |= 0x0000555a;
	BK3000_AE_CONFIG &= ~(1<<16);
	BK3000_AE_CONFIG |= (1<<16);
}


void rf_test_user_clear_reg(void)
{
	BK3000_AE_CONFIG &= ~0x0000fffe;
	BK3000_AE_CONFIG &= ~(1<<16);
	BK3000_AE_CONFIG |= (1<<16);
}


uint32 rf_test_user_read_reg(void)
{
	if ((BK3000_AF_CONFIG & 0x0000fffe) == 0x0000555a)
		return 1;
	else
		return 0;
}
...
void User_Eeprom_Btaddr_Write(uint8 * Pbtaddr)
{
	uint32 file_size;
	...
	{	
        flash_erase_sector(FLASH_CHANGED_BT_ADDR_4M);
        flash_write_data_user(Pbtaddr, FLASH_CHANGED_BT_ADDR_4M, 6);
	}
	else
	{
        flash_erase_sector(FLASH_CHANGED_BT_ADDR_8M);
        flash_write_data_user(Pbtaddr, FLASH_CHANGED_BT_ADDR_8M, 6);
	}
}


void User_Eeprom_Btaddr_Read(uint8 * Pbtaddr)
{
	uint32 file_size;
	...
	if (file_size == FLASH_ENVSIZE_4M_FLAG)
	{
		flash_read_data(Pbtaddr, FLASH_CHANGED_BT_ADDR_4M, 6);
	}
	else
	{
		flash_read_data(Pbtaddr, FLASH_CHANGED_BT_ADDR_8M, 6);
	}
}


void User_Eeprom_CRC_Read(uint8 * Pcrc)
{
	uint32 file_size;
	...
	if (file_size == FLASH_ENVSIZE_4M_FLAG)
	{
		flash_read_data(Pcrc, FLASH_CHANGED_BT_ADDR_4M-4, 4);
	}
	else
	{
		flash_read_data(Pcrc, FLASH_CHANGED_BT_ADDR_8M-4, 4);
	}
}


void User_Eeprom_Vlcf_Read(void)
{
	uint32 block_num;
	uint32 flash_addr;
	uint8 eeprom_data[EEPROM_BLOCK_SIZE];
	uint8 flash_data[EEPROM_WHOLE_SIZE];

	if (flash_get_4M_flag())
		flash_addr = FLASH_ENVDATA2_DEF_ADDR_4M + 2048;
	else
		flash_addr = FLASH_ENVDATA2_DEF_ADDR_8M + 2048;

	flash_read_data((uint8 *)&flash_data[0], flash_addr, EEPROM_WHOLE_SIZE);
	flash_addr = 0;

	/* 查找有效的地址，最多EEPROM_BLOCK_NUMBER */
	for (block_num = 0; block_num < EEPROM_BLOCK_NUMBER; block_num++)
	{	
		memcpy((uint8 *)&eeprom_data[0], (uint8 *)&flash_data[flash_addr], EEPROM_BLOCK_SIZE);	
		if ((eeprom_data[0] == EEPROM_BLOCK_ID1) && (eeprom_data[1] == EEPROM_BLOCK_ID2))
			break;

		flash_addr += EEPROM_BLOCK_SIZE;
	}

	if (block_num >= EEPROM_BLOCK_NUMBER)
	{
		/* 首次无数据或写入超范围补偿值无效 */
	...
	{
		if ((eeprom_data[2] + eeprom_data[3]) == 0xff)
			user_batt_charger_vlcf = eeprom_data[2];
		else
			user_batt_charger_vlcf = 0xff;

		eeprom_block_num = block_num;
	}
}


void User_Eeprom_Vlcf_Write(uint8 vlcf)
{
	uint8 eeprom_data[4];
	uint32 flash_addr;
	...
	if (eeprom_block_num <= EEPROM_BLOCK_NUMBER - 1)
	{	
		/* 清除当前有效标志 */
		eeprom_data[0] = 0;		//id1
		eeprom_data[1] = 0;		//id2
		flash_addr = flash_addr + eeprom_block_num * EEPROM_BLOCK_SIZE;
        flash_write_data_user((uint8 *)&eeprom_data[0], flash_addr, 2);

		/* 保存在下1个BLOCK */
		eeprom_data[0] = EEPROM_BLOCK_ID1;
		eeprom_data[1] = EEPROM_BLOCK_ID2;
		eeprom_data[2] = vlcf;
		eeprom_data[3] = 0xff - vlcf;
		
		flash_addr += EEPROM_BLOCK_SIZE;
        flash_write_data_user((uint8 *)&eeprom_data[0], flash_addr, EEPROM_BLOCK_SIZE);
		eeprom_block_num++;		
	}
}
