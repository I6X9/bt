...
#define SERIAL_NUMBER     "12345"// "FC58A1005588"//
#define DEV_NAME           "KSAITK"
#define DEV_TYPE           "A2SSNYIXMY2O2V"
...
void Add_Pause_Cmd(void)
{
	...
	if(( app_h->flag_sm1 & APP_BUTTON_FLAG_PLAY_PAUSE)) //当前是播放
	{

		flag_pause = 1; //标志置1，交互之前被暂停了，交互结束后要重新播放
...
#define ALI_MIC_ANA_GAIN		66
#define ALI_MIC_DIG_GAIN		63
...
int  audio_start(void)
{
	...
	//if(voice_start_flag == VOICE_RECORD_FREE)
	{
		...
		app_set_led_event_action( LED_EVENT_RESERVED1 );
...
**************************************************************/
AudioProfile SAMPLE_ALIGN audio_prolfie1[3]={AUDIO_PROFILE__CLOSE_TALK,AUDIO_PROFILE__NEAR_FIELD,AUDIO_PROFILE__FAR_FIELD};
AudioFormat SAMPLE_ALIGN audio_format1[1]={AUDIO_FORMAT__OPUS_16KHZ_16KBPS_CBR_0_20MS};
AudioSource SAMPLE_ALIGN audio_source1[1]={AUDIO_SOURCE__STREAM};

void ble_get_setting(SpeechSettings*p)
{
   p->audio_profile=audio_prolfie1[1];//(AudioProfile)0;
   p->audio_format=audio_format1[0];//(AudioFormat)2;
   p->audio_source=audio_source1[0];//(AudioSource)0;
}
...
char SAMPLE_ALIGN serial_num[13] ;
...
{
	DeviceBattery *battery1;
	DeviceStatus *status1;

	app_env_handle_t  env_h = app_env_get_handle();

	memset(serial_num,0,13);
	sprintf(serial_num,"%02x%02x%02x%02x%02x%02x",
	...
	p->device_id=(uint32_t)0x34345678;
	p->battery=battery1;
	p->status=status1;
	p->product_color=100;
}
...
void ble_get_central_info(GetCentralInformation*p)
{
