...
#endif
uint8 flash_get_16M_flag(void)
{
    unsigned int flash_flag;

    switch (flash_mid)
    {
        case 0xb4015: //xtx 16M flash 
            flash_flag = 1;
            break;
        default:
            flash_flag = 0;
...
void flash_write_data_user (uint8 *buffer, uint32 address, uint32 len) {
    uint32 i, reg_value;
    uint32 addr = address&(~0x1f);
    uint32 buf[8];
    uint8 *pb = (uint8 *)&buf[0];
