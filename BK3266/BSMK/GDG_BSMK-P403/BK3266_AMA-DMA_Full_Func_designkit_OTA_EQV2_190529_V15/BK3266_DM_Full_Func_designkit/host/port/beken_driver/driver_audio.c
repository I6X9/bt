...
void aud_mute_update_led( int16 samplel, int16 sampler )
{
...
#endif
{
        ...
        if(sts  & bit_ADCL_INTR_FLAG)
        {
            ...
            //while(!(sts & (bit_ADCL_FIFO_EMPTY|bit_DACR_FIFO_FULL|bit_DACL_FIFO_FULL)))
            {
                ...
                else
                {
                    ...
                    pcmval = (pcmval & 0xffff0000 ) | ((uint16) pcm_tmp & 0xffff) ;


				#ifdef USER_AUX_EQ
					aux_ptr.in_ptr[aux_ptr.ptr_cnt] = pcmval;
					pcmval = aux_ptr.out_ptr[aux_ptr.ptr_cnt];	
				#endif

	                if(flag_L_is_LplusR)
	                {
	                    pcmbuf[0] = (int16) (pcmval & 0x0000ffff);
	                    pcmbuf[1] = (int16) ((pcmval>>16) & 0x0000ffff);

	                    pcmval = (int32)(pcmbuf[0] + pcmbuf[1]) >> 1;
	                    pcmbuf[0] = (int16) (pcmval & 0x0000ffff);
						pcmbuf[1] = pcmbuf[0] * (-1);
				...
				#ifdef USER_AUX_EQ
					aux_ptr.ptr_cnt++;
					if (aux_ptr.ptr_cnt >= AUX_BUF_BYTE_SIZE/4)
					{
						aux_eq_deal = 1;
						aux_ptr.ptr_cnt = 0;
						if (aux_ptr.buf_cnt == 0)
						{
							aux_ptr.buf_cnt = 1;
							aux_ptr.in_ptr =   aux_ptr.ptr_buf2;
							aux_ptr.deal_ptr = aux_ptr.ptr_buf0;
							aux_ptr.out_ptr =  aux_ptr.ptr_buf1;
						}
						else if (aux_ptr.buf_cnt == 1)
						{
							aux_ptr.buf_cnt = 2;
							aux_ptr.in_ptr =   aux_ptr.ptr_buf1;
							aux_ptr.deal_ptr = aux_ptr.ptr_buf2;
							aux_ptr.out_ptr =  aux_ptr.ptr_buf0;
						}
						else if (aux_ptr.buf_cnt == 2)
						{
							aux_ptr.buf_cnt = 0;
    ...
    if(sts  & bit_ADCL_INTR_FLAG)
    {
        ...
        {
            aud_linein_fill_buffer( (uint8 *)&pcm_data32[0], size*2*2 );
        }
    #endif
    }
}
...
void RAM_CODE wav_fill_buffer( uint8 *buff, uint16 size )
{
	...
	if (flag_L_is_LplusR)
		LeadelityMono((short*)tmp_buf, (short*)tmp_buf, nSmpls/2, 44100, 0);
	else
		Leadelity((short*)tmp_buf, (short*)tmp_buf, nSmpls/2, 44100, 0);
#endif
	...
	int32 p;
	int16 *ptr;
	int32 *data_tmp = (int32 *)jmalloc(nSmpls*sizeof(uint32), 0);
	...
	if((aud_pre_eqe.online_flag == 0x5a)||(aud_pre_eqe.online_flag == 1))
	{
		...
		func_sw_eq((int32 *)data_tmp,(int32 *)data_tmp, nSmpls/2, aud_pre_eqe.totle_EQ,0);
	...
	else if(app_drc.flag)
	{
		...
		if((app_drc.flag)&(1<<2))
			drc_apply(&app_drc, (int16_t*)tmp_buf, (int16_t*)tmp_buf, nSmpls/2);
...
{
#ifdef CONFIG_DRIVER_DAC
    //dac_volume_mute(enable);
#endif

#ifdef CONFIG_DRIVER_I2S
    i2s_volume_mute(enable);
...
CONST int32 table_ex_eq_vol[1+16] = 
{
	0x8e, //gain=-119.5
	0x679f2, //gain=-50.0
	0x147ae1, //gain=-40.0
...
extern void volumeSet(int vol);

void app_ex_eq_setvol(uint8 vol)
{
	if (vol > AUDIO_VOLUME_MAX)
		vol = AUDIO_VOLUME_MAX;

#if (USER_EX_EQ == 1)
	volumeSet(table_ex_eq_vol[vol]);
#endif
}
