...
//    bt_app_ctx_h app_ctx)
{
	...
	uint16_t i=0;
	uint16 spp_time_out=0;


    sniffmode_wakeup_dly = 1000;		//ÑÓÊ±½øsniff
...
#endif	
   app_ota_up_action(0x1002,(unsigned char *)&result[0], length);

    if((flag_ota_enable)&&(otaupflag))
        app_ota_spp_pdu_reframe((unsigned char *)&result[0], length);
    else
#endif
	{
		 ...
		 if(length<4)
		 return;
		 if(((uint8)result[i]!=0x10)&&((uint8)result[i+1]!=0))
			return;
		while (i<length)
			{
				   if(((uint8)result[i]==0x10)&&((uint8)result[i+1]==0))
					{
				put_que(result[i+2]+3,(uint8*)&result[i]); 
				i=i+result[i+2]+3;
