...
extern int dueros_dma_stop_speech(void);
...
#else
{
	...
	#ifdef BT_DUALMODE
		//SCO开始前，BLE没有连接时关闭BLE广播，降低BLE广播对传统BT的丢包干扰
	    if(! LEconnection_LE_Connected())
			flag_ble_enable = 0;
	#endif


		//普通SCO通话下的MIC模拟增益使用配置值
	    aud_mic_volume_set(env_h->env_cfg.system_para.vol_mic);

		//普通SCO通话下的MIC数字增益使用配置值
...
result_t util_sco_disconnect(sco_utils_h priv)
{
    ...
    os_printf("*****aili_sco_active clear_4\r\n");
