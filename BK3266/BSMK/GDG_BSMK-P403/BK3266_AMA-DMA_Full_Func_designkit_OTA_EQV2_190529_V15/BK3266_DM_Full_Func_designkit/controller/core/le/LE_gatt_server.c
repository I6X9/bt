...
uint8 ble_revece_ok=0;
uint16 ble_revece_leng=0;
...
#ifdef BEKEN_OTA_BLE
const GATT_DB_t GATT_DB_SELECT[DB_LEN]={USER_DEF_FFA0_GATT_DB, USER_DEF_OAD_GATT_DB};
#else
const GATT_DB_t GATT_DB_SELECT[DB_LEN]={USER_DEF_FFA0_GATT_DB};
#endif //BEKEN_OTA
    ...
    *p_result_len = 0x00;
    ...
    while ((GATTserver_DB[i].attrib_handle <= end_handle) && ((*p_result_len+*p_attrib_val_len) <= (max_result_len-7)) && (i<=GATTserver_DB_last_index))
    {
        ...
        if (p_current_attrib->attrib_UUID.UUID_val == GATT_ATT_TYPE_CHARACTERISTIC)
        {
            ...
            // UUID for 獵haracteristic?and Attribute Value set to the Characteristic Properties,
...
u_int8 GATTserver_Find_Descriptor_Information(u_int16 max_result_len,u_int16 start_handle,u_int16 end_handle,u_int8* p_result,u_int8* p_result_len,u_int8* p_format)
{
    ...
    while ((GATTserver_DB[i].attrib_handle <= end_handle) && (i<=GATTserver_DB_last_index))
    {
        t_attrib* p_current_attrib = &GATTserver_DB[i];
        {
             ...
             * Response with the 獻nsufficient Authentication? 獻nsufficient Authorization?
             * 獻nsufficient Encryption Key Size?or 獳pplication Error?error code.
             */
...
/*******user defined code****************
 * Get_Data_from_DB
 *  This function is used to read a characteristic value or a characteristic descriptor value from local Server DB.
 ...
 * Parameters:
 * INPUTS
 *  u_int8* p_attrib_value     :- A pointer to the data read,
...
void Get_Data_from_DB(u_int8* p_attrib_value, u_int8 length, u_int8 index)//获取数据
{	
    sniffmode_wakeup_dly = 1000;		//延时进sniff
	bt_flag2_operate(APP_FLAG2_WAKEUP_DLY, 1);

    //os_printf("data_length==%d\r\n",length);

#if 1
	if(GATTserver_DB[index].attrib_handle == user_define_notify_handle_rx)
	{

	  if(length<4)
	 return;
	 if(((uint8)p_attrib_value[0]!=0x10)&&((uint8)p_attrib_value[1]!=0))
	 	return; 
     	 dueros_process_cmd_from_phone((uint8*)p_attrib_value,length);		
	}
#endif
...
extern uint8 send_dma_ver;
extern u_int8 data_buffer_rx[160];
...
//#ifdef LE_DEBUG_ENABLE
 //   os_printf("GATTserver_Write_Command: handle:%x, p_value: %s, length %d",attrib_handle,p_attrib_value, length);
//#endif	


	if(attrib_handle == (user_define_notify_handle_tx+1) 
		&& (p_attrib_value[0] == 0x01) 
	  && (p_attrib_value[1] == 0x00)
	  && (length == 2))
	{
		send_dma_ver=1;
		//os_printf("nifity_ok\r\n");
        ...
        // Now write the Value...
        {
            ...
            //if (length <= value_len)
            {
                ...
                for (j=0;j<length;j++)
                {
					...
					data_buffer_rx[j]= p_attrib_value[j];	
			
                    // os_printf("p_value[%d] = %x\r\n",j,p_value[j]);
                ...
                if ((!descriptor) & (p_current_attrib->attrib_value.value.local_notification))//for ATT_WRITE command
                {
                    if (status == GATT_SERVER_HANDLE_WRITE_RESP)
                    {
                        ...
                        LEatt_Encode_PDU(GATT_Get_Active_Handle(),ATT_WRITE_RESPONSE,&pdu_params);
			   			os_printf("write_reesp\r\n");
                    }
                    else
                    {
                    	GATT_Event(GATT_WRITE_COMPLETE_EVENT,attrib_handle,length, 0,p_value, NO_ERROR);
                    	//os_printf("val = %d\r\n",p_attrib_value[0]);
				//if(GATTserver_DB[i].attrib_handle == user_define_notify_handle_rx)
				//{
					//ble_revece_leng=length;
					//ble_revece_ok=1;
				//}
                    }
#ifdef BEKEN_OTA_BLE
		    ...
		    {
		    	 //ota_update_flag = 1;
                    	 app_ota_ble_pdu_decode(GATTserver_DB[i].attrib_handle, p_value, length);
		    }
		...
		else
			{
				   ...
				   os_printf("write_reesp\r\n");
	                    	...
	                    	{
                                os_printf("write_nooreesp\r\n");
                                if(GATTserver_DB[i].attrib_handle == user_define_notify_handle_rx)
                                {
                                    //ble_revece_leng=length;
                                    //ble_revece_ok=1;
                                    put_que(length,(uint8*)data_buffer_rx);
                                }
...
extern uint8 frame_audio_flag;//////1----audio,2-----cmd,0---------nodata
#endif
...
t_GATT_error GATTserver_Characteristic_Write_Local_Value(u_int16 char_val_handle,u_int16 length, u_int8* p_attrib_value)
{
    ...
    u_int16 char_UUID;
		...
		if( (BTq_Get_Queue_Number_Of_Entries(L2CAP_OUT_Q, p_connection->device_index) + \	
			BTq_Get_Queue_Number_Of_Entries(LMP_OUT_Q, p_connection->device_index))>(PRH_BS_CFG_SYS_NUM_OUT_ACL_PACKETS-5))	
			{
				if(frame_audio_flag==1)
				{
					os_printf("\r\n !!!!!!!more than threshold!!!!\r\n");
...
void app_gatt_write_local_value(u_int16 char_val_handle,u_int8 length, u_int8* p_attrib_value)
{
    GATTserver_Characteristic_Write_Local_Value(char_val_handle, length, p_attrib_value);	
}
