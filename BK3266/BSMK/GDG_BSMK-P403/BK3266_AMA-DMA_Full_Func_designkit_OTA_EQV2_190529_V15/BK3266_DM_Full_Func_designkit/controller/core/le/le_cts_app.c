...
//#define long_device_name_support        1


uint8 LE_name_addr[4];
...
void LE_APP_Init(void)
{
  ...
  //  GAP_Set_AD_Data(14,(u_int8*)Find_ME_adv_data);

#if 1


//当LE名称大于26字节时需分包发送
#if 0//def long_device_name_support

    app_env_handle_t  env_h = app_env_get_handle();
	u_int8 data[64];
	u_int8 split_adv_flag = 0;
	u_int8 i, name_leng, data_leng,LE_name_addr_idx;

	name_leng = string_size((uint8*)&env_h->env_cfg.bt_para.device_name[0]);

	// AD_DATA LEN MAX = 31,
	// NAME LENG MAX(18) = 31 - LE_name_addr(4BYTE)-  Flags(3 BYTE) -Services (4 BYTE) - NAMElen(1BYTE) - NAMEflag(1BYTE)
	// AD_DATA NAME LEN MAX 22 = 31 - Services (4 BYTE) - Flags (3 BYTE) - NAMETYPE(1BYTE)- NAMElen(1BYTE)
	// SCAN RSP_DATA LEN MAX = 31, NAME LEN MAX(29) = 31 - NAMElen(1BYTE) - NAMEflag(1BYTE)
	// TOTAL NAME MAX LEN ( name_leng )= 29 + 18
	...
	if (1 == split_adv_flag)
	{
		...
		/* Services //0x03 data length,0x02 16-bits UUIDS incomplete */
		data[data_leng++] = 0x03;
		data[data_leng++] = 0x02;
		data[data_leng++] = 0x66;
		data[data_leng++] = 0x66;
		...
		{
			data[data_leng++] = env_h->env_cfg.bt_para.device_name[i];
		}

#if 1
		for(i = 0,LE_name_addr_idx = 0; i < 22 - name_leng; i++)
		{
			data[data_leng++] = LE_name_addr[3 - i];
			LE_name_addr_idx++;
		}
#endif

		GAP_Set_AD_Data(data_leng, &data);

		/********** GAP_Set_ScanResp_Data ***********/
		data_leng = 0;

		/* ble name */
		data[data_leng++] = name_leng - 22 + 1 + 4;
		data[data_leng++] = 0x09;

#if 1
		for(i = 0; i < name_leng - 22 + 4; i++)
		{
			data[data_leng++] = LE_name_addr[3 - i - LE_name_addr_idx];
		}
#endif

		GAP_Set_ScanResp_Data(data_leng, &data);
	}
	else if (2 == split_adv_flag)
	{
		/********** GAP_Set_AD_Data ***********/
		data_leng = 0;
		...
		for(i = 0; i < name_leng - 22 + 4; i++)
		{
			data[data_leng++] = env_h->env_cfg.bt_para.device_name[22];
		}

		data[data_leng++] = LE_name_addr[3];
		data[data_leng++] = LE_name_addr[2];
		data[data_leng++] = LE_name_addr[1];
		data[data_leng++] = LE_name_addr[0];
    ...
    for(i = 0; i < 6; i++)
    {
	 data[data_leng++] = env_h->env_cfg.bt_para.device_addr.b[5-i];
    }
	#endif

    GAP_Set_AD_Data(data_leng, (u_int8*)&data[0]);
	...
	/* ble name */
#if (AMA_ENABLE == 1)
	data[data_leng++] = name_leng+1+2;
#else

	#ifdef TIANMAO_ENABLE
	data[data_leng++] = name_leng+1+4;	
	#else
	data[data_leng++] = name_leng+1;//+4;
	#endif
    ...
    GAP_Set_ScanResp_Data(data_leng, (u_int8*)&data[0]);
    ...
    case  GATT_WRITE_COMPLETE_EVENT:
    {
    	 ...
    	 //os_printf("handle = %x\r\n",p_gatt_event_params->handle);
              switch(p_gatt_event_params->handle)
           	 {
      		    //extern void gpio_config( int index, int dir );
      		    //extern void gpio_output( int index, uint32 val );
				...
				//temp_gpio = p_gatt_event_params->payload[0];
           		    	...
           		    	//EQ_Data_Slave_converter(p_gatt_event_params->payload,aud_equ_ble);
                            //gpio_config(temp_gpio, 1);
                           // gpio_output(temp_gpio, 1);
...
{
  // ble_online=1;
   le_connected_first = 120;
   os_printf("Notify_BLE_Connected\r\n");

}
void Notify_BLE_Disconnected(void)
{
   os_printf("Notify_BLE_Disconnected\r\n");
  // ble_online=0;
}
...
u_int16 App_Setup_GAP_GATT_DB(void)
{
    ...
    for(i = 0; i < name_leng; i++)
    {
		gap_name_array[data_leng++] = env_h->env_cfg.bt_para.device_name[i];
	}

	//gap_name_array[data_leng++] = LE_name_addr[3];
	//gap_name_array[data_leng++] = LE_name_addr[2];
	//gap_name_array[data_leng++] = LE_name_addr[1];
	//gap_name_array[data_leng++] = LE_name_addr[0];

#if (AMA_ENABLE == 1)
	gap_name_array[data_leng++] = 'L';
	gap_name_array[data_leng++] = 'E';
#endif

	gap_name_array[data_leng++] = 0;					// 0 is name end

	if (data_leng > 31)
		data_leng = 31;	

    _GATTserver_Add_Characteristic_Value(handle,DEVICE_NAME,NO_LOCAL_NOTIFY,GATT_PERMISSION_READ |GATT_PERMISSION_WRITE, data_leng, gap_name_array);
...
u_int8 user_define_str[11]={"BLE_TART"};
u_int8 data_buffer_rx[160];
u_int8 data_buffer_tx[160];
u_int8 user_define_device_description_rx[2]= {0x01,0x00};
u_int8 user_define_device_description_tx[2]= {0x01,0x00};

u_int8 user_define_notify_handle_rx;
u_int8 user_define_notify_handle_tx;


#if(AMA_ENABLE==1)
CONST static u_int8 dma_sever_uuid[16] = {0xFB,0x34,0x9B,0x5F,0x80,0x00,0x00,0x80,0x00,0x10,0x00,0x00,0x03,0xFE,0x00,0x00};
CONST static u_int8 dma_rx_uuid[16] = {0x76,0x30,0xF8,0xDD,0x90,0xA3,0x61,0xAC,0xA7,0x43,0x05,0x30,0x77,0xB1,0x4E,0xF0};
CONST static u_int8 dma_tx_uuid[16] = {0x0B,0x42,0x82,0x1F,0x64,0x72,0x2F,0x8A,0xB4,0x4B,0x79,0x18,0x5B,0xA0,0xEE,0x2B};
#else
CONST static u_int8 dma_sever_uuid[16] = {0xFB,0x34,0x9B,0x5F,0x80,0x00,0x00,0x80,0x00,0x10,0x00,0x00,0x04,0xFE,0x00,0x00};
CONST static u_int8 dma_tx_uuid[16] = {0x0f,0x33,0x84,0x47,0x53,0x9d,0xa1,0xbb,0xd4,0x46,0xc5,0x29,0xc6,0xc9,0x4a,0xb8};
CONST static u_int8 dma_rx_uuid[16] = {0xc5,0x3f,0x19,0x93,0xa5,0x98,0xcb,0xb0,0xe0,0x41,0x78,0x0e,0xb9,0x58,0xe7,0xc2};
#endif
...
os_printf("FFA0_INt\r\n");
    ...
    _GATTserver_Add_Service(PRIMARY_SERVICE_16BIT, handle,0x00,0xFAA0);
    handle++;
    //handle = 0xC1;
    					/**************EQ_Param0*************/
    _GATTserver_Add_Characteristic_Declaration(handle,0xFAA1,0x16);//Characteristic Properties// bit 0 :Broadcast, bit 1 : Read , bit 2 : Write Without Response, bit 3 : Write
    // bit 4 : Notify, bit 5 : Indicate, bit 6 : Authenticated Signed Writes,
    //bit 7 : Extended Properties
    handle++;

    _GATTserver_Add_Characteristic_Value(handle,0xFAA1,USE_LOCAL_NOTIFY,GATT_PERMISSION_READ|GATT_PERMISSION_WRITE,sizeof(data_buffer),data_buffer);
    user_define_notify_handle = handle;
	handle++; //handle = 0xC3;

    _GATTserver_Add_Char_Descriptor(handle,CLIENT_CHARACTERISTIC_CONFIGURATION,GATT_PERMISSION_READ|GATT_PERMISSION_WRITE,sizeof(user_define_device_description),user_define_device_description);//handle == 0x00B3 == 179
    handle++; //handle = 0xC3;

    _GATTserver_Add_Char_Descriptor(handle,CHARACTERISTIC_USER_DESCRIPTION,GATT_PERMISSION_READ,sizeof(user_define_str),user_define_str);
    handle++; //handle = 0xC4;
...
#if(AMA_ENABLE==1)
    u_int16 handle = 0xf0;
    _GATTserver_Add_Service(PRIMARY_SERVICE_128BIT,handle,dma_sever_uuid,0x0000);
    handle++;


    _GATTserver_Add_16ByteUUID_Characteristic_Declaration(handle,dma_rx_uuid,0x08);
    handle++;
    _GATTserver_Add_16ByteUUID_Characteristic_Value(handle,dma_rx_uuid,NO_LOCAL_NOTIFY,GATT_PERMISSION_READ|GATT_PERMISSION_WRITE,sizeof(data_buffer_rx),data_buffer_rx);

    user_define_notify_handle_rx = handle;
     handle++;;
_GATTserver_Add_Char_Descriptor(handle,CLIENT_CHARACTERISTIC_CONFIGURATION,GATT_PERMISSION_READ|GATT_PERMISSION_WRITE,0x02,user_define_device_description_rx);

    handle++;


    _GATTserver_Add_16ByteUUID_Characteristic_Declaration(handle,dma_tx_uuid,0x12);
    handle++;
    _GATTserver_Add_16ByteUUID_Characteristic_Value(handle,dma_tx_uuid,NO_LOCAL_NOTIFY,GATT_PERMISSION_READ|GATT_PERMISSION_WRITE,sizeof(data_buffer_tx),data_buffer_tx);

    user_define_notify_handle_tx = handle;
    handle++;
_GATTserver_Add_Char_Descriptor(handle,CLIENT_CHARACTERISTIC_CONFIGURATION,GATT_PERMISSION_READ|GATT_PERMISSION_WRITE,0x02,user_define_device_description_tx);
  handle++;;
    ...
    _GATTserver_Add_16ByteUUID_Characteristic_Declaration(handle,dma_rx_uuid,0x0C);
    handle++;
    _GATTserver_Add_16ByteUUID_Characteristic_Value(handle,dma_rx_uuid,NO_LOCAL_NOTIFY,GATT_PERMISSION_READ|GATT_PERMISSION_WRITE,sizeof(data_buffer_rx),data_buffer_rx);
...
_GATTserver_Add_Char_Descriptor(handle,CLIENT_CHARACTERISTIC_CONFIGURATION,GATT_PERMISSION_READ|GATT_PERMISSION_WRITE,0x02,user_define_device_description_rx);
    ...
    _GATTserver_Add_16ByteUUID_Characteristic_Declaration(handle,dma_tx_uuid,0x10);
    handle++;
    _GATTserver_Add_16ByteUUID_Characteristic_Value(handle,dma_tx_uuid,NO_LOCAL_NOTIFY,GATT_PERMISSION_READ|GATT_PERMISSION_WRITE,sizeof(data_buffer_tx),data_buffer_tx);
